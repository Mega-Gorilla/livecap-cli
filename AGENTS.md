...
- `engines/metadata.py` defines `EngineMetadata.default_params` as the single source of truth for engine defaults.
...
- Document engine-specific options in `engines/metadata.py` via `EngineMetadata.default_params`.
