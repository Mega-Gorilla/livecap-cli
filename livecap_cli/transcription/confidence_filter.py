...
- ``engine_confidence.is_available is False`` の engine (qwen3asr) は
  無条件 pass-through (fail-open)。fail-open は意図的な設計で、未対応 engine
  が silently 全 reject されることを防ぐ。Voxtral は PR-A.4.1 ([#311]) から
...
- threshold は実機 verify 済の値を default (WhisperS2T: 0.5、Parakeet (ja/en)
  ...
  ReazonSpeech: -0.2 [PR-A.5.1 smoke verify 2026-06-11])。``FilterConfig``
  ...
  で global avg_logprob 経路を完全 opt-out 可能)。
- PR-A.5.1 (Issue #317) で engine-specific threshold dict
  ...
  dict に load せず global ``avg_logprob_threshold = -1.0`` fallback で
  backward compat 維持。
...
class FilterConfig:
    ...
    Attributes:
        ...
        avg_logprob_thresholds: **engine-specific** ``avg_logprob`` threshold の
            ...
            dict に load しない (= ``avg_logprob_threshold = -1.0`` を使用、
            backward compat)。ReazonSpeech は ``-0.2`` を default load
...
def should_reject(
    ...
    if (
        ...
        if engine_name is not None:
            threshold = config.avg_logprob_thresholds.get(engine_name)
        if threshold is None:
            threshold = config.avg_logprob_threshold
        if threshold is not None and ec.avg_logprob < threshold:
            engine_tag = f" (engine={engine_name})" if engine_name else ""
            return True, (
...
def apply_filter(
    ...
    *,
    ...
    - ``engine_confidence.is_available is False`` の engine
      (ReazonSpeech / qwen3asr) は無条件 pass-through。Voxtral は
      PR-A.4.1 ([#311]) から avg_logprob を populate するため filter 対象
      (strict-gated、``should_reject`` docstring 参照)。Canary は
      PR-A.4.2 ([#311]) から token_confidence_mean を populate するため
      filter 対象 (Parakeet_ja と同 path 共用)。
