...
- ``mode`` が ``"observe"`` の時は判定 + structured log のみ、reject はしない
  (PR-A.3 calibration 用、本番動作なし)。
- ``mode`` が ``"on"`` (default) の時は reject 時に ``None`` を返し、log 出力。
- ``engine_confidence.is_available is False`` の engine (ReazonSpeech / qwen3asr /
  voxtral / canary) は無条件 pass-through (fail-open)。fail-open は意図的な
  設計で、未対応 engine が silently 全 reject されることを防ぐ。
- threshold は PR-A.0 実機 verify 済の値を default (WhisperS2T: 0.5、
  Parakeet_ja: 0.005)。``FilterConfig`` の constructor kwargs で override 可能
  (sweep harness が PR-A.3 で programmatic に threshold を sweep するため)。
...
def apply_filter(
    ...
    *,
    ...
    - ``"observe"``: 判定 + structured log のみ、result はそのまま返す。
    - ``"on"``: 判定 + log + reject 時に ``None`` 返却 (silent drop)。

    fail-open:

    - ``engine_confidence.is_available is False`` の engine
      (ReazonSpeech / qwen3asr / voxtral / canary) は無条件 pass-through。
    ...
    if rejected:
        logger.info(
            "confidence_filter[%s]: source=%s engine=%s decision=%s reason=%s "
            "text=%r ec=%s",
            config.mode,
            decision.source_id,
            decision.engine,
