...
- ``engine_confidence.is_available is False`` の結果は pass-through
  ...
  **ReazonSpeech は PR-A.5.1 ([#317]) から ``avg_logprob`` (sherpa-onnx の
  ...
  を populate (両言語 en/ja で Phase 1 probe 確認)。**
- threshold は **[#334] PR-4 で Phase 2 report ([calibration-japan-engines-phase2-2026-07.md]
  §4.1) の Pareto gate 適用値を default に採用** (WhisperS2T: ``0.71``、
  Parakeet (ja/en) / Canary: ``0.001``、Voxtral: ``-1.0`` [PR-A.4.1 smoke
  verify 2026-06-11、 Phase 2 未 calibrate で不変]、ReazonSpeech: ``-0.40``、
  qwen3asr: ``-0.42``)。旧 default (PR-A.0 / PR-A.5.X smoke verify 値、
  ``0.5`` / ``0.005`` / ``-0.2`` / ``-0.3``) から Phase 2 augmented corpus
  1375 sample の再 calibration 結果に基づく更新。``FilterConfig``
...
- PR-A.5.1 (Issue #317) で engine-specific threshold dict
  (``avg_logprob_thresholds: Dict[str, float]``) を追加。Voxtral と
  ReazonSpeech / qwen3asr は同 ``avg_logprob`` field を共用するが分布が
  桁違いのため engine-specific threshold が必要 ([#334] PR-4 で Phase 2
  report §4.1 の Pareto 値に更新: ReazonSpeech ``-0.40`` / qwen3-asr
  ``-0.42``)。``voxtral`` は
...
class FilterConfig:
    ...
    Attributes:
        ...
        no_speech_threshold: WhisperS2T の ``no_speech_prob`` が **これより上** なら reject。
            ...
            **Default: ``0.71``** ([#334] PR-4 で Phase 2 report §2.3 Pareto relaxed_B
            適用、旧 ``0.5`` から更新)。公式 Whisper default ``0.6`` 近傍。 Phase 2
            augmented corpus 1375 sample で F1=0.901 / clean_frr 2.67% / SNR≥5 全て ≤ 2%。
            旧 ``0.5`` は PR-A.0 実機 verify 値 (Phase 1 report §2、 speech ``0.036``
            vs non-speech ``0.66`` で 100% 分類可能) だが、 Layer 2/3 augmented corpus
            では desk_tap/applause 系の ``0.63``-``0.66`` を pass させる trade-off で
            clean speech recall を上げるほうが Pareto gate 通過に有利。

            公式 Whisper は加えて ``avg_logprob <= -1.0`` との **AND logic** で skip
            判定するが、livecap-cli は ``no_speech_prob`` 単独で reject (strict gate
            設計、本 module docstring の「strict gating ルール」参照)。AND logic への
            移行は Issue #334 Finding 4 で継続検討。
        token_conf_threshold: Parakeet/Canary の ``token_confidence_mean`` が **これより
            下** なら reject。

            **Default: ``0.001``** ([#334] PR-4 で Phase 2 report §2.4 Pareto strict
            適用、旧 ``0.005`` から更新)。 Phase 2 で Parakeet_ja F1=0.961 / clean 0.67% /
            false reject 39→11 (72% 削減)、 non_speech recall 97.9%→94.1% の trade-off。
            collateral: Parakeet_en / Canary も同 scalar を共用 (Phase 2 未 calibrate だが
            speech mean 実測から新 threshold ``0.001`` 対しても margin 十分)。

            engine 別 speech mean 実測 (Phase 1 report §2 F2、 参考値):

            - Parakeet ja (CTC): ≈ ``0.0504`` (新 threshold から 50× margin)
            - Parakeet en (TDT): ≈ ``0.2452`` (新 threshold から 245× margin)
            - Canary en (AED): ≈ ``0.0724`` (新 threshold から 72× margin)
        ...
        avg_logprob_thresholds: **engine-specific** ``avg_logprob`` threshold の
            ...
            **Default entries** ([#334] PR-4 で Phase 2 report §4.1 Pareto 値に更新):

            - ``reazonspeech``: ``-0.40`` (Phase 2 §2.1 Pareto relaxed_B、 旧
              PR-A.5.1 default ``-0.2`` の FRR 42.5% 実害を 5.4% に改善)
            - ``qwen3-asr``: ``-0.42`` (Phase 2 §2.2 Pareto relaxed_C、 JA/EN 両方
              に単一 dict key で適用、 旧 PR-A.5.2 default ``-0.3`` から変更)
