...
- ``engine_confidence.is_available is False`` の engine (ReazonSpeech /
  qwen3asr) は無条件 pass-through (fail-open)。fail-open は意図的な
  設計で、未対応 engine が silently 全 reject されることを防ぐ。Voxtral は
  PR-A.4.1 ([#311]) から ``avg_logprob`` を populate するため filter 対象
  (下記 strict gate)。Canary は PR-A.4.2 ([#311]) から
  ``token_confidence_mean`` を populate するため filter 対象 (Parakeet_ja と
  同 ``token_conf_threshold`` を共用)。Parakeet 英語は PR-A.4.3 ([#316])
  から同 ``token_confidence_mean`` を populate する (NeMo TDT +
  ``preserve_alignments`` 経由、Parakeet_ja と同 helper を流用)。
- threshold は実機 verify 済の値を default (WhisperS2T: 0.5、Parakeet (ja/en) /
  Canary: 0.005、Voxtral: -1.0 [PR-A.4.1 smoke verify 2026-06-11])。
  ``FilterConfig`` の constructor kwargs で override 可能 (sweep harness が
  PR-A.3 で programmatic に threshold を sweep するため、
  ``avg_logprob_threshold=None`` で Voxtral 経路を完全 opt-out 可能)。
