...
本 doc は [Issue #334](https://github.com/Mega-Gorilla/livecap-cli/issues/334) audit の成果物として作成された。6 engine の confidence_filter 統合を audit した結果、**docs と code の乖離が構造的に発生していた**こと (stale docstring / scale 誤認 / silent fail-open) が判明、また「量子化 calibration 観点を docs に明文化する」 観点も追加で重要と認識された。これらを **anti-pattern として codify** することで再発を防止するのが本 doc の目的。
...
| Step 4 (20-70%) | `_get_or_download_model()` → `_download_model()` (abstract) | `NotImplementedError` | **HuggingFace repo の flattened dir が正本なら `RepoDirModelMixin` を使う** (`livecap_cli/engines/repo_dir_engine.py`、Qwen3-ASR / WhisperS2T / Voxtral / ReazonSpeech): `class FooEngine(RepoDirModelMixin, BaseEngine)` にして `_repo_dir_spec()` で `RepoDirSpec(repo_id, required, variant, allow_patterns, ignore_patterns, legacy_subdirs, dir_name_override, legacy_names)` を返すだけで、Step 3 の path (`<models_root>/<org>--<name>/`)、cache hit (`validate_repo_dir` = manifest + required)、旧配置の取り込み (`_reconcile_legacy_layouts` → `migrate_dir`)、取得 (`_download_model` → `fetch_repo_dir`: staging → manifest → atomic publish) が揃う。`_load_model_from_path()` では `self._require_model_dir(path)` で確認したローカル dir を渡し、load 失敗時は `self._invalidate_model_dir(path, reason=)` で self-heal する。**`_is_model_cached` / `_verify_model_integrity` を engine 側で override しない** (`tests/core/engines/test_repo_dir_engine.py` が固定)。単一ファイル (NeMo の `.nemo`) は `hf_cache.download_file()` + `_reconcile_legacy_layouts` で `legacy_model_layouts.migrate_nemo_file(validate=self._verify_model_integrity)`。**ライブラリ任せの download (`from_pretrained(<repo id>)` / `load_model("base")`) は既定 cache へ落ちて管理外になる** (#428 / #430 / #447 / #456、`docs/architecture/model-store-contract.md`) |
...
"my_engine": EngineInfo(
    ...
    cli_default_language="ja",                 # --language 未指定時の実効値
    supports_language_auto=False,          # native 自動検出対応時のみ True
)
```

> **`cli_default_language` の制約 (Issue #365)**: dataclass 互換 default は `""` だが、
...
> 解決する — 未指定 → `cli_default_language`、`auto` → `supports_language_auto`
> の engine のみ許可、非対応言語はモデルロード前に `ValueError`。
> なお `cli_default_language` は **CLI policy** であり、engine constructor の
> `language=` default と一致させる必要はない (qwen3asr は意図的に
> constructor=auto / CLI=ja — PR-A.5.2 の設計判断)。

> **言語データの正本規約 (Issue #230)**: adapter の `get_supported_languages()`
> に言語 list を **hardcode してはならない**。正本は次のいずれか:
>
> | パターン | 正本 | 派生 | 採用例 |
> |---|---|---|---|
> | 通常 (推奨) | `EngineMetadata` の登録 entry | adapter は `list(EngineMetadata.get(self.engine_name).supported_languages)` を返す | canary / voxtral / reazonspeech / parakeet |
> | 言語 map が必要な engine | data-only module (`*_languages.py`、heavy import 禁止) | metadata とadapter の両方が同 module から派生 | whispers2t (`whisper_languages.py`) / qwen3asr (`qwen3asr_languages.py`) |
>
> `EngineInfo.supported_languages` は `__post_init__` で **tuple 化**される
> (外部 mutation の封鎖)。adapter↔metadata の一致は
> `tests/core/engines/test_language_authority.py` が CI で強制する — 新 engine
> 追加時は同テストの parametrize に行を追加すること。
...
| `reazonspeech` | ReazonSpeech K2 v2 | AvgLogprob | sherpa-onnx `ys_log_probs` mean | `< -0.2` (engine-specific) | なし | **int8 / float32 両方 verified** (PR-A.5.1、margin +0.13 / +0.10、両方 Case A — `docs/research/reazonspeech-confidence-smoke-2026-06-11.md` 参照) |
| `qwen3asr` | Qwen3-ASR 0.6B | AvgLogprob | HF `compute_transition_scores`、**language-specified 時のみ** | `< -0.3` (engine-specific、両言語同一) | なし (refuse / system-prompt-like output は observed behavior / hypothesis、公式 threshold なし、Issue #334 Finding 7 参照) | (HF default) |
...
### AP-3: 量子化 (int8 / float32) を smoke verify せず threshold 採用する (F8 由来、一般則)

**問題**: 量子化は logits 計算精度に影響し、`avg_logprob` 等の signal 分布を変える可能性がある。片方の量子化だけで calibration して採用すると、もう片方で false reject が起きる risk あり。

**Issue #334 audit での議論**: 量子化軸を audit 観点として明示的に挙げた。既存 engine では ReazonSpeech が PR-A.5.1 で **int8 / float32 両方 smoke verify 済** (margin +0.127 / +0.105、両方 Case A、`docs/research/reazonspeech-confidence-smoke-2026-06-11.md` および `docs/audio-filter-reference.md:316` に記録)。本 anti-pattern は **既存実装の gap ではなく、新規 engine 追加時の必須 checklist として** documenting する。

**正解** (新 engine 追加時):
- §5.4 に従い **両量子化 (or その engine が expose する全 quantization option) で smoke verify**
- 同分布なら 1 threshold 共用 (ReazonSpeech 方式)、分布差大なら量子化別 threshold (`avg_logprob_thresholds` dict の key を `"engine:int8"` 等に拡張) を検討
- calibration data の量子化情報 (測定 model / margin / Case 判定) を **`docs/research/{engine}-confidence-smoke-{date}.md` のような research doc に残す** (ReazonSpeech が precedent)
- threshold docstring に「量子化別 verify 済」or「量子化軸は scope 外」を明記
...
- [ ] **`EngineInfo` に `cli_default_language` (+ 必要なら `supports_language_auto=True`) を明示** (Issue #365、§3.1 の制約参照。`tests/core/engines/test_metadata.py::TestLanguageResolutionMetadata` の期待表にも新 engine を追加)
- [ ] **言語データの正本規約に従う** (Issue #230、§3.1 参照): `get_supported_languages()` に hardcode せず metadata or data-only module から派生 + `tests/core/engines/test_language_authority.py` の parametrize に新 engine を追加
