...
| Step 4 (20-70%) | `_get_or_download_model()` → `_download_model()` (abstract) | `NotImplementedError` | **必ず実装**。HuggingFace 由来なら `livecap_cli/engines/hf_cache.py` を使う — `fetch_repo_dir()` (repo の必要ファイルを staging 経由で `<models_root>/<org>--<name>/` へ flattened dir + `livecap-manifest.json` として publish。Qwen3-ASR / WhisperS2T / Voxtral / ReazonSpeech) か `download_file()` (単一ファイルを管理 staging 経由で models root へ。NeMo の `.nemo`)。cache hit は `model_store.validate_repo_dir()` (manifest の全ファイルがサイズ一致で実在) だけで決める — `_is_model_cached` / `_verify_model_integrity` を override するならこれを呼ぶ。旧配置の取り込みは `legacy_model_layouts.migrate_dir()` / `migrate_nemo_file()` を `_download_model()` の先頭で呼ぶ。**ライブラリ任せの download (`from_pretrained(<repo id>)` / `load_model("base")`) は既定 cache へ落ちて管理外になる** (#428 / #430 / #447 / #456、`docs/architecture/model-store-contract.md`)。ローカル dir を `_load_model_from_path()` へ渡し、load 失敗時は `invalidate_manifest()` で self-heal する |
