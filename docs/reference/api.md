...
if result.success:
    ...
    # 例: "All 3 ASR segment calls failed; first error: TypeError: ..."
    print(f"Failed: {result.error}")
```

> **注意**: `segment_transcriber` は文字列を返すこと。`engine.transcribe()` の戻り値は
> `TranscriptionResult` であり、tuple unpack / index アクセス (`[0]`) は不可 (#314 で
> `__iter__` 削除済み)。`.text` を取り出して返す。
