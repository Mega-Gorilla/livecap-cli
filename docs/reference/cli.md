...
| `--confidence-filter` | Engine confidence filter mode ([#308] PR-A.1)。`off` で PR-A.0 挙動に戻す。`observe` は判定を JSON log するが reject しない (PR-A.3 calibration 用)。`on` は WhisperS2T `no_speech_prob > 0.5` / Parakeet_ja `token_confidence_mean < 0.005` / Voxtral `avg_logprob < -1.0` (PR-A.4.1 [#311]、strict-gated: 他 signal 不在時のみ) / Canary `token_confidence_mean < 0.005` (PR-A.4.2 [#311]、greedy decoding 経由) で reject (silent drop)。confidence 信号を持たない engine (ReazonSpeech / qwen3asr) は常に pass-through (fail-open)。env var `LIVECAP_CONFIDENCE_FILTER` が **CLI flag より優先**。詳細は [`audio-filter-reference.md`](../audio-filter-reference.md) §5。 | `on` |
