...
| `--confidence-filter` | Engine confidence filter mode ([#308] PR-A.1)。`off` で PR-A.0 挙動に戻す。`observe` は判定を JSON log するが reject しない (PR-A.3 calibration 用)。`on` は WhisperS2T `no_speech_prob > 0.5` / Parakeet (ja/en) / Canary `token_confidence_mean < 0.005` (ja: PR-A.0 CTC、en: PR-A.4.3 [#316] TDT + preserve_alignments、Canary: PR-A.4.2 [#311] greedy decoding 経由) / Voxtral `avg_logprob < -1.0` (PR-A.4.1 [#311]、strict-gated: 他 signal 不在時のみ) / ReazonSpeech `avg_logprob < -0.2` (PR-A.5.1 [#317]、engine-specific threshold) / **qwen3asr `avg_logprob < -0.3`** (PR-A.5.2 [Issue #318]、wrapper bypass + `repetition_penalty=1.1 + no_repeat_ngram_size=3` 経由、両言語 en/ja で confirmed) で reject (silent drop)。**7 engine 対応で PR-A 系列完成**。env var `LIVECAP_CONFIDENCE_FILTER` が **CLI flag より優先**。詳細は [`audio-filter-reference.md`](../audio-filter-reference.md) §5。 | `on` |
