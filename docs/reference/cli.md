...
| `LIVECAP_TRANSLATION_TIMEOUT` | 1 segment の翻訳を待つ秒数。未設定時は **`5.0` 秒** (Issue [#402] で `10.0` から変更 — リアルタイム字幕では遅れて届いた翻訳は今話している内容と重なるだけで価値が無い)。超過した segment は原文のまま出て `translation_state="failed"` になり、次の segment は前の翻訳が終わるまで `skipped_busy` になる。回線が遅い環境では上げる。 | 例: `LIVECAP_TRANSLATION_TIMEOUT=10` |
...
> **モデルの正本は `LIVECAP_CORE_MODELS_DIR` です** (`docs/architecture/model-store-contract.md`、[#456](https://github.com/Mega-Gorilla/livecap-cli/issues/456))。`LIVECAP_CORE_CACHE_DIR` は staging / lock / 一時ファイルだけで、消してもモデルは失われません (0.2.0 では Qwen3-ASR / WhisperS2T の snapshot が `LIVECAP_CORE_CACHE_DIR/huggingface/hub` にありましたが、0.3.0 で `models_root` へ移ります)。**HuggingFace 由来のモデル (Qwen3-ASR / WhisperS2T の CTranslate2 snapshot、reazonspeech / voxtral の HF download) は 0.2.0 時点では `LIVECAP_CORE_CACHE_DIR/huggingface/hub` に落ちます** (`livecap-cli info` の `HF cache` 行、Issue [#428](https://github.com/Mega-Gorilla/livecap-cli/issues/428) / [#430](https://github.com/Mega-Gorilla/livecap-cli/issues/430))。NeMo (canary / parakeet) の `.nemo` は `LIVECAP_CORE_CACHE_DIR/downloads/` を経由して `LIVECAP_CORE_MODELS_DIR/<org>--<name>.nemo` に 1 部だけ置かれます ([#447](https://github.com/Mega-Gorilla/livecap-cli/issues/447))。`HF_HOME` / `HF_HUB_CACHE` は参照しません。
