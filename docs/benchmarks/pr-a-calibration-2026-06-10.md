...
- **🆕 PR-A.4.3** (TBD): **Parakeet 英語** の filter 対応。旧 docs では「NeMo RNNT path に token_confidence 未実装 → 構造的限界」と PR-A.5 candidate に分類していたが、PR-A.4.docs ([#316](https://github.com/Mega-Gorilla/livecap-cli/pull/316)) 作業中の調査で **「実は PR #309 時点の `preserve_alignments=True` 併設漏れ」**と判明。実機 probe で `token_confidence_mean = 0.2452` (LibriSpeech 英語、threshold 0.005 の **49×**) を確認、PR-A.4.3 candidate に格上げ済。
- **PR-A.5** (TBD): ReazonSpeech / qwen3asr の **wrapper/upstream 側制約**対応 (sherpa-onnx upstream PR or PyTorch native 実装切替 / wrapper bypass or vLLM logprobs 移行、heavy track)。Finding 5 で実機確認した「ReazonSpeech は fail-open で filter 効果なし」を解消する track。
