...
`laughing` は全 engine で recall 63-70% と最下位、 `breathing` は engine 差が大きい (ReazonSpeech 73% vs Parakeet_ja 97%)。 いずれも音響的に speech に極近く、 「reject すべき non_speech」 と定義するかが微妙。 **本 report の判断**: Layer 3 noise source からの除外 (元 A 案) は Phase 2b で保留、 代わりに **metric 分離** で対応する:
