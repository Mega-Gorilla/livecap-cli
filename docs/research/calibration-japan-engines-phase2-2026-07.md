...
3. **PR-4 用 threshold candidate は "relaxed Pareto gate" 適用で選定**:
   ...
   - parakeet_ja: **0.001** (strict Pareto pass、 clean 0.7%、 F1=0.961) or 現状 0.005 維持
4. **量子化 (int8 vs float32) は threshold に影響しない** — ReazonSpeech で完全同一 threshold `-0.37` (F1 max criterion)。 Finding F8 の一般則を実測 confirm。
5. **breathing / laughing は engine 独立に難** (recall 70-73%)、 これらは Layer 3 noise source から除外検討 (§5.4)。
...
Phase 2 で新規発生:
- **breathing / laughing 除外検討**: 全 engine で recall 70-73%、 speech に音響的に極近。 Layer 3 noise source として妥当か再検討 (§5.4)
...
| parakeet_ja | 0.005 | 0.001 | 0.006 | **`0.001`** or 現状 `0.005` 維持 | -0.004 or 0 | strict (0.001 で clean 0.67%) |
...
| parakeet_ja | `0.001` | strict (clean 0.67%) | 0.961 |
...
### 4.4 Downstream (Issue #334 の他 PR との関係)
...
### 5.8 Layer 3 noise diversity limit (breathing + car_horn のみ実測)
...
### 5.7 F1 max criterion は残す (Pareto gate の baseline として)
...
| **PR-4 で採用可能な threshold** | reazon `-0.40` / qwen3 `-0.42` / whispers2t `0.71` / parakeet_ja `0.001` (or 現状 `0.005` 維持) |
