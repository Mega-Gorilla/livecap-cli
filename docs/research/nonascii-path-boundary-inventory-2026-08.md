...
- 棚卸し行数: **44**、未分類 (決定なし): **0**
- 実測レコード数: **126**
- **決定** の内訳: ②wide-path 36 行 / ③staging 3 行 / ④fail-fast 2 行 / 非該当 3 行
- **実測で確定** している applicable 行: **38 / 41** — ②wide-path 34 行 / ③staging 3 行 / ④fail-fast 1 行 / 未確定 3 行
...
| 呼び出し元 | 渡すパス | 受け側ライブラリ | wide path 対応 | 非 ASCII 実測 | 失敗の可視性 | 決定 | **実測で確定** | 粒度 | 追跡 |
|---|---|---|---|---|---|---|---|---|---|
| `livecap_cli/engines/parakeet_engine.py:437` | なし (ndarray in / ndarray out) | librosa | n/a | — 対象外 | — | 非該当 | — 対象外 | - | — |
