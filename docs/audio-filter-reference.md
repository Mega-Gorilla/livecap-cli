...
|---|---|---|---|
| **whispers2t** | `no_speech_prob` | `> 0.71` reject ([#334] PR-4 で Phase 2 report §2.3 Pareto relaxed_B に更新、 旧 `> 0.5`) | Real-machine: speech 0.036 (pass) vs non-speech 0.63-0.66 (drop). 20× separation. Phase 2 Pareto relaxed_B pass (clean_frr 2.67%、 Whisper 公式 0.6 近傍)。 |
...
| **reazonspeech** | `avg_logprob` (engine-specific threshold) | `< -0.40` reject ([#334] PR-4 で Phase 2 report §2.1 Pareto relaxed_B に更新、 旧 `< -0.2`) | **PR-A.5.1 [#317] real-machine smoke (2026-06-11)**: speech mean -0.14 (int8) / -0.16 (float32) vs non-speech mean -0.30 / -0.45 → margin +0.13 (int8) / +0.10 (float32)、 両 model で Case A clean separation。 sherpa-onnx 1.12.39 で `OfflineRecognitionResult.ys_log_probs` を取得、 mean を `EngineConfidence.avg_logprob` (Voxtral と同 semantics) に populate。 Section 2 で `webrtc × real × on` の Hall.(post) **50% → 0%** を実証 (Issue #295 元 motivation の最後の cell 完了)。 **engine-specific threshold** (`avg_logprob_thresholds["reazonspeech"] = -0.40`) で Voxtral 用 `-1.0` と分離。 **Phase 2 report §2.1 で旧 default `-0.2` の FRR 42.5% 実害を新 default `-0.40` で 5.4% に改善**。 |
...
|---|---|---|---|---|---|
| **WhisperS2T** | ✅ Production | `no_speech_prob` | `> 0.71` ([#334] PR-4、 旧 `> 0.5`) | [#309/#310] + [#334] PR-4 | 20× (speech 0.036 vs non-speech 0.66)、 Pareto relaxed_B |
| **Parakeet_ja** | ✅ Production | `token_confidence_mean` | `< 0.001` ([#334] PR-4、 旧 `< 0.005`) | [#309/#310] + [#334] PR-4 | 167× (speech 0.05 vs non-speech 0.0000029)、 Pareto strict (F1=0.961) |
...
| **ReazonSpeech** | ✅ **Production** | `avg_logprob` (engine-specific threshold) | `< -0.40` reject ([#334] PR-4、 旧 `< -0.2`) | **[#317] PR-A.5.1** + [#334] PR-4 | **margin +0.13 (int8) / +0.10 (float32)、 両方 Case A、 Pareto relaxed_B で FRR 42.5%→5.4%** |
