...
| **When to tune** | Per-engine thresholds は **[#334] PR-4 で Phase 2 report ([`calibration-japan-engines-phase2-2026-07.md`](research/calibration-japan-engines-phase2-2026-07.md)) の Pareto gate 適用値** に更新済 (旧 PR-A.0/A.5.X smoke verify 値から permissive 方向へ変化、 詳細は §4.1)。 Override programmatically via `FilterConfig(no_speech_threshold=..., token_conf_threshold=..., avg_logprob_threshold=...)` (no CLI flag yet; PR-A.3 calibration doc 参照)。 |

### Engine support

| Engine | Filter material | Threshold (default) | Behavior with `--confidence-filter on` |
|---|---|---|---|
| **whispers2t** | `no_speech_prob` | `> 0.71` reject ([#334] PR-4 で Phase 2 report §2.3 Pareto relaxed_B に更新、 旧 `> 0.5`) | Phase 1 real-machine (historical、 旧 threshold `0.5` 時代): speech 0.036 vs non-speech 0.63-0.66 で clean separation。 **[#334] PR-4 で threshold `0.71` に緩和した後は、 non-speech probe `0.63`-`0.66` は意図的に pass** させる Pareto trade-off (§2.3 relaxed_B、 clean_frr 2.67% / SNR≥5 全て ≤ 2% / F1=0.901、 Whisper 公式 0.6 近傍と整合)。 clean speech (`0.036`) は依然 pass、 Layer 2 hard-negative の一部を pass する引き換えに clean speech recall を上げる設計。 |
| **parakeet_ja** | `token_confidence_mean` | `< 0.001` reject ([#334] PR-4 で Phase 2 report §2.4 Pareto strict に更新、 旧 `< 0.005`) | Real-machine: speech 0.01-0.10 (pass) vs non-speech 0.0000029-0.0003 (drop). 3-4 orders of magnitude separation. Phase 2 Pareto strict pass (F1=0.961、 false reject 39→11 = 72% 削減、 non_speech recall 97.9%→94.1% の trade-off allow)。 |
| **voxtral** | `avg_logprob` (strict-gated) | `< -1.0` reject (変更なし) | PR-A.4.1 real-machine smoke (2026-06-11): speech mean -0.42 (pass) vs non-speech mean -1.53 (drop). Margin +1.0, midpoint -1.02. Strict-gated: only evaluated when `no_speech_prob` and `token_confidence_mean` are both `None` — so WhisperS2T / Parakeet_ja never enter this path. Phase 2 未 calibrate (global fallback として維持)。 |
| **canary** | `token_confidence_mean` | `< 0.001` reject ([#334] PR-4 collateral、 旧 `< 0.005`) | PR-A.4.2 real-machine smoke (2026-06-11): native English speech mean 0.0724 (pass, 新 threshold `0.001` から 72× margin)。 Greedy decoding + `confidence_cfg.preserve_token_confidence` 経由で NeMo `multitask_greedy_decoding.pack_hypotheses` から `torch.Tensor` token_confidence を取得、`.tolist()` で list 化して mean 計算。 日本語など非対応言語入力では engine 自体が empty text を返す fail-safe (filter は介入不要)。 Phase 2 未 calibrate だが scalar collateral scope で speech margin 十分 (実害なし)。 |
| **parakeet (英語)** | `token_confidence_mean` | `< 0.001` reject ([#334] PR-4 collateral、 旧 `< 0.005`) | PR-A.4.3 [#316] real-machine smoke (2026-06-11): native English speech mean **0.2452** (pass、 新 threshold `0.001` から **245× margin**)。 NeMo TDT decoding + `preserve_alignments=True` + `confidence_cfg.preserve_token_confidence` 経由で `hypothesis.token_confidence` (List[float]) を取得、 Parakeet_ja と同 helper で mean 計算。 Section 2 stream pipeline で `webrtc × synthetic × on` の Hall.(post) 75% → 12.5% を実証。 非英語入力 (日本語等) では language mismatch による低 confidence で false reject の可能性、 `--confidence-filter off` で opt-out 可能。 Phase 2 未 calibrate だが scalar collateral scope で speech margin 超余裕 (実害なし)。 |
| **reazonspeech** | `avg_logprob` (engine-specific threshold) | `< -0.40` reject ([#334] PR-4 で Phase 2 report §2.1 Pareto relaxed_B に更新、 旧 `< -0.2`) | Phase 1 real-machine (historical、 旧 threshold `-0.2` 時代、 **PR-A.5.1 [#317] smoke 2026-06-11**): speech mean -0.14 (int8) / -0.16 (float32) vs non-speech mean -0.30 / -0.45、 旧 threshold `-0.2` に対し margin +0.13 (int8) / +0.10 (float32) の Case A clean separation。 **[#334] PR-4 で threshold `-0.40` に緩和した後は、 int8 non-speech mean `-0.30` は意図的に pass** させる Pareto trade-off (§2.1 relaxed_B、 clean_frr 2.9% / SNR≥5 全て ≤ 4% / F1=0.929)。 sherpa-onnx 1.12.39 で `OfflineRecognitionResult.ys_log_probs` を取得、 mean を `EngineConfidence.avg_logprob` (Voxtral と同 semantics) に populate。 Section 2 で `webrtc × real × on` の Hall.(post) **50% → 0%** を旧 threshold で実証 (Issue #295 元 motivation の最後の cell 完了)。 **engine-specific threshold** (`avg_logprob_thresholds["reazonspeech"] = -0.40`) で Voxtral 用 `-1.0` と分離。 **Phase 2 report §2.1 で旧 default `-0.2` の FRR 42.5% 実害を新 default `-0.40` で 5.4% に改善**。 |
| **qwen3-asr** | `avg_logprob` (engine-specific threshold) | `< -0.42` reject ([#334] PR-4 で Phase 2 report §2.2 Pareto relaxed_C に更新、 旧 `< -0.3`) | **PR-A.5.2 ([Issue #318]) real-machine smoke (2026-06-12)**: 両言語 verified — EN: speech mean -0.05、 non-speech -0.71、 margin **+0.65** / JA: speech mean -0.12、 non-speech -0.63、 margin **+0.42**、 両言語 Case A clean separation。 **Wrapper bypass で `Qwen3ASRForConditionalGeneration.generate(output_scores=True, repetition_penalty=1.1, no_repeat_ngram_size=3)` を直接呼び**、 `compute_transition_scores(normalize_logits=True)` 経由で per-token logprob を取得、 Voxtral 同 helper で mean 計算。 **`repetition_penalty=1.1 + no_repeat_ngram_size=3`** で両言語の failure mode (EN: system prompt leak、 JA: 256-token repetition loop) を完全解消、 Section 2 で Hall.(pre) 0% 全 cell (Canary と同 engine 固有 fail-safe pattern)。 `_asr_language is None` (auto-detect mode) と auto-detect 用 fail-open path 残置。 WER 軽微退行 (LLM typical 0.5-1%) は filter benefit 優先で allow。 `--confidence-filter off` は **post-ASR reject のみ** 無効化し、 `repetition_penalty=1.1 + no_repeat_ngram_size=3` の generation 側変更は固定で残る (Voxtral greedy / Canary greedy と同 framing)。 Phase 2 は JA calibration → EN は同 dict key で `-0.42` 適用 (Phase 1 EN probe で safety verify 済、 speech -0.05 依然 pass / applause -1.08 依然 reject)。 SNR 10 borderline は Layer 4 production observe で再確認予定。 |
...
```
Confidence filter: ON (whispers2t no_speech_prob > 0.71, parakeet (ja/en) / canary token_conf < 0.001, voxtral avg_logprob < -1.0, reazonspeech avg_logprob < -0.40, qwen3-asr avg_logprob < -0.42). Disable: --confidence-filter off or LIVECAP_CONFIDENCE_FILTER=off
```

The `voxtral avg_logprob < -1.0` clause is omitted when the user explicitly opts out by passing `FilterConfig(avg_logprob_threshold=None)` (PR-A.4.1). The `parakeet (ja/en) / canary` clause shows the shared `token_conf_threshold` used by Parakeet_ja (TDT-CTC hybrid)、 **Parakeet 英語** (TDT only, PR-A.4.3 [#316])、 Canary (AED multitask, PR-A.4.2) — all three populate `EngineConfidence.token_confidence_mean` via NeMo greedy decoding with `preserve_token_confidence=True`. The **`reazonspeech avg_logprob < -0.40`** and **`qwen3-asr avg_logprob < -0.42`** clauses are the engine-specific thresholds ([#334] PR-4 で Phase 2 report reflected values、 `FilterConfig.avg_logprob_thresholds["reazonspeech"] / ["qwen3-asr"]`)、 separated from Voxtral's `-1.0` because the two engines' avg_logprob distributions differ by an order of magnitude.
...
|---|---|---|---|---|---|
| **WhisperS2T** | ✅ Production | `no_speech_prob` | `> 0.71` ([#334] PR-4、 旧 `> 0.5`) | [#309/#310] + [#334] PR-4 | Historical (旧 threshold `0.5`): 20× (speech 0.036 vs non-speech 0.66)。 Current (`0.71` Pareto relaxed_B): clean_frr 2.67% / SNR≥5 全て ≤ 2% |
| **Parakeet_ja** | ✅ Production | `token_confidence_mean` | `< 0.001` ([#334] PR-4、 旧 `< 0.005`) | [#309/#310] + [#334] PR-4 | Historical (旧 threshold `0.005`): 167× (speech 0.05 vs non-speech 0.0000029)。 Current (`0.001` Pareto strict): F1=0.961、 false reject 39→11 (72% 削減)、 non_speech recall 97.9%→94.1% |
| **Voxtral** | ✅ Production (strict-gated) | `avg_logprob` | `< -1.0` (変更なし) | [#313] PR-A.4.1 | +1.0 (speech mean -0.42 vs non-speech mean -1.53) |
| **Canary** | ✅ Production | `token_confidence_mean` | `< 0.001` (Parakeet 共用、 [#334] PR-4 collateral) | [#315] PR-A.4.2 + [#334] PR-4 | 72× (speech 0.0724 vs 新 threshold 0.001) |
| **Parakeet (英語)** | ✅ **Production** | `token_confidence_mean` | `< 0.001` (Parakeet 共用、 [#334] PR-4 collateral) | **[#316] PR-A.4.3** + [#334] PR-4 | **245× (speech 0.2452 vs 新 threshold 0.001)** |
| **ReazonSpeech** | ✅ **Production** | `avg_logprob` (engine-specific threshold) | `< -0.40` reject ([#334] PR-4、 旧 `< -0.2`) | **[#317] PR-A.5.1** + [#334] PR-4 | Historical (旧 threshold `-0.2`): margin +0.13 (int8) / +0.10 (float32)。 Current (`-0.40` Pareto relaxed_B): clean_frr 2.9% / SNR≥5 全て ≤ 4%、 **FRR 42.5%→5.4%** |
| **qwen3-asr** | ✅ **Production** | `avg_logprob` (engine-specific threshold、 wrapper bypass) | `< -0.42` reject ([#334] PR-4、 旧 `< -0.3`) | **[#318] PR-A.5.2** + [#334] PR-4 | **両言語 verified — EN +0.65 / JA +0.42 margin、 両方 Case A、 Pareto relaxed_C** |
| mock | — (test fixture only) | — | — | — | — |
...
5. **`--confidence-filter=on`** (default since PR-A.1). Provides a
   ...
   - ReazonSpeech: `avg_logprob` で **margin +0.10-0.13** (PR-A.5.1 [PR #319] smoke int8/float32 + 12 cell stream pipeline、 engine-specific threshold `-0.40` — [#334] PR-4 で Phase 2 report §2.1 Pareto relaxed_B に更新、 旧 `-0.2`)
   - **qwen3-asr**: `avg_logprob` で **両言語 margin +0.42〜+0.65** (PR-A.5.2 [Issue #318] smoke EN+JP + 12 cell stream pipeline、 wrapper bypass + `repetition_penalty=1.1 + no_repeat_ngram_size=3`、 engine-specific threshold `-0.42` — [#334] PR-4 で Phase 2 report §2.2 Pareto relaxed_C に更新、 旧 `-0.3`)
