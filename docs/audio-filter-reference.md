...
| Parakeet (英語) | 🆕 **未対応 → PR-A.4.3 candidate** | `token_confidence_mean` (probe で 0.2452 確認) | (将来 < 0.005 想定) | (TBD、Issue #311 v2.2) | 49× (probe sample、要 sweep) |
...
#### Parakeet 英語の判明経緯 (PR-A.4.docs probe 中に発覚)
...
4. **実機 probe (本 PR docs 作業中)**: Path 1 (Hybrid CTC) と同じ pattern (preserve_alignments + preserve_frame_confidence + confidence_cfg) を非 hybrid model に適用 → **Parakeet 英語で `token_confidence_mean = 0.2452` populate を確認**

→ 「構造的限界」ではなく **「設定漏れ」**だった。Issue #311 v2.2 として **PR-A.4.3** で正式対応 (本 PR は docs PR scope で discipline のため対応見送り)。

### Production user の選択 (engine と filter benefit の対応)

| User の engine 選択 | Confidence filter benefit |
|---|---|
| WhisperS2T / Parakeet_ja / Voxtral / Canary | ✅ 自動 hallucination 抑制 (default on で active) |
| Parakeet 英語 | 🆕 PR-A.4.3 で対応予定 (probe 済、現状は fail-open) |
