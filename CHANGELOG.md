...
- **棚卸し表** `docs/research/nonascii-path-boundary-inventory-2026-08.md` — ネイティブ / 第三者ライブラリへパスを渡す **44 境界**を列挙し、境界ごとに ①buffer / ②wide-path / ③ASCII staging / ④fail-fast / 非該当 のいずれかへ分類 (**未分類ゼロ**)。`ascii_safe_path()` の契約 (シグネチャ / 生存期間 / 機構 ladder / staging root / 並行ロード契約 / fail-loud / 却下した代替案) もここに確定
