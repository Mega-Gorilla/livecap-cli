...
- **Added**: `ffmpeg.path_env` probe。**`PATH` に挿した非 ASCII の bin ディレクトリから、basename だけで実行ファイルを解決できるか**を測る。`transcription.file_pipeline.ffmpeg_binary` は**フルパスを渡して**起動するので PATH 探索を通らず、**あちらの pass では代用できない** (レビュー指摘)。`cjk_kana` / `outside_acp` を必須にした
