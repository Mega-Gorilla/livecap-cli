...
- **Removed**: **`unicode_safe_download_directory()`**。`%TEMP%` を `cache_root/downloads/<uuid>` へ移設するだけで、**その `cache_root` はユーザー名を含み得るため ASCII 保証が無い** (棚卸し §5.1 で実測)。`unicode_safe` を名乗る名前自体が「これを使えば ASCII 安全」という誤読を招く。shim は残さない (pre-1.0)
  ...
  - **Migration**: `boundary` は**必須キーワード引数**である (失敗メッセージの 1 番目に出る診断契約)。内部の呼び出し 5 箇所 — `engine.parakeet.from_pretrained` / `engine.canary.from_pretrained` / `engine.qwen3asr.from_pretrained` / `engine.reazonspeech.download_int8` / `engine.reazonspeech.download_float32` — をすべて移した
...
- **`%TEMP%` を消費しない scope から wrapper を外すことはしていない。** `reazonspeech` の 2 経路は `download_file()` が `cache_root/downloads` へ直接書き、`temporary_directory()` と `snapshot_download()` は行き先を明示するので `%TEMP%` を通らない。それでも 1:1 置換に留めたのは、外すには「この scope 配下が `%TEMP%` を一切消費しない」の証明が要り、それが第三者ライブラリの内部実装に依存するため。個別の除去は実測の根拠を用意して別途判断する
