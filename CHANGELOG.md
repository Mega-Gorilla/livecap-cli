...
- **Added**: **lease** による使用中判定。「TTL 超過かつ `rmtree` が通る」は生存判定ではない — 使用中のプロセスがその瞬間ハンドルを開いていなければ消せてしまい、`%TEMP%` を継承した子プロセスが後から使う場合も判別できない。**スコープの全期間にわたって開いたままのファイル**を entry の中に置き、Windows では `rmtree` の `PermissionError` が、POSIX では `flock` が判定になる。**lease を entry の外に置くと `rmtree(entry)` を妨げず、Windows の保護そのものが消える**ため、reaper の単位 (entry) と消費側に見せるディレクトリ (その子) を分けて「空のディレクトリを返す」契約と両立させた
