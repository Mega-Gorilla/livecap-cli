...
#### `hf_cache.resolve_snapshot()` と `*.marker`、ReazonSpeech の tarball 経路、`get_models_dir(engine_name)`、`ModelManager.download_file()` を削除 ([#456])

- **Before**: Qwen3-ASR / WhisperS2T は `hf_cache.resolve_snapshot()` で `<cache_root>/huggingface/hub` の hub 階層へ解決し、`<models_root>/<org>--<name>.marker` に snapshot の相対 path を書いていた (`read_marker` / `write_marker` / `invalidate_marker`)。ReazonSpeech int8 は GitHub の tarball (713 MB) を `download_file()` + `tarfile` で展開していた。`ModelManager.download_file()` / `download_file_async()` は `urllib.request.urlretrieve` で `<cache_root>/downloads/<filename>` へ直接書く helper で、ReazonSpeech の tarball 経路が消えた後は利用者が無く、ModelRoot 契約 (staging → manifest → 原子的 publish) の外にあった。`BaseEngine._download_model(target_path, progress_callback, model_manager=None)` の第 3 引数は test 用の注入口で production では常に `self.model_manager` だった。`ModelManager.get_models_dir(engine_name)` / `utils.get_models_dir(engine_name)` は `<models_root>/<engine_name>/` を作って返し、parakeet / reazonspeech の `load_model()` override がそこへ正本を移してから template を呼ぶ (template は root 側で miss して再ダウンロード) **inverted workaround** の入口になっていた。canary は `_prepare_model_directory()` override が `.nemo` の path を dir として返し、`<name>.nemo/<name>.nemo` を作っていた
- **After**: `hf_cache.fetch_repo_dir()` (staging → flattened dir + manifest → 原子的 publish) と `model_store` (`validate_repo_dir` / `adopt_dir` / `publish_dir` / `invalidate_manifest`)、`legacy_model_layouts` (`migrate_dir` / `migrate_nemo_file` / `scan_legacy_layouts`) に置き換え。flattened dir が正本の 4 engine は `repo_dir_engine.RepoDirModelMixin` (`_repo_dir_spec()` で `RepoDirSpec` を返すだけ) を共有し、cache 判定 / 取り込み / 取得の実装は 1 つ。marker 関数、tarball 経路、`load_model()` / `_prepare_model_directory()` の override、`get_models_dir()` の `engine_name` 引数、`ModelManager.download_file()` / `download_file_async()`、`_download_model()` の `model_manager` 引数を削除。`BaseEngine._is_model_cached()` / `_verify_model_integrity()` は dir を manifest だけで判定する (「非空 dir」は hit ではない)
- **Migration**: `resolve_snapshot` / marker 関数を使っていたコードは `fetch_repo_dir(repo_id, hub_root=, staging_root=, destination=, allow_patterns=, required=)` へ。`get_models_dir("engine")` は `get_models_dir()` へ (engine subdir は作られない)。`ModelManager.download_file(url)` を使っていたコードは `hf_cache.download_file` (HF の単一ファイル) か `resources.downloader` (retry 付き HTTP、FFmpeg 用) へ。engine の `_download_model(self, target_path, progress_callback)` は 2 引数 (manager は `self.model_manager`)。既存の marker / hub snapshot / engine subdir は初回ロードで自動的に取り込まれる。nonascii harness の `engine.reazonspeech.snapshot_download` / `engine.reazonspeech.tarfile_extract` 行と `huggingface_hub.local_files_only` / `huggingface_hub.snapshot_download.write` / `tarfile.extractall` probe は callsite が消えたので削除し、`huggingface_hub.snapshot_download.local_dir.write` (production `fetch_repo_dir()` を mock Hub 相手に通す) に置き換えた
...
- **After**: `legacy_model_layouts.external_hub_roots()` (`huggingface_hub.constants.HF_HUB_CACHE` と `platformdirs.user_cache_dir("whisper_s2t")/models`) の `models--<org>--<name>/snapshots/…` を旧配置の候補の**最後**に加え、cold load で必要ファイルだけを copy (同一 volume なら hardlink) して `<models_root>/<org>--<name>[.nemo]` へ publish する (Riva も同じ経路。OPUS-MT は変換元 snapshot を staging へ copy して変換する)。manifest には由来の commit sha (`snapshots/<sha>` の名前) を残す。root の中の候補があればそちらを優先する。**外の cache は他アプリと共用なので削除しない** (`LegacyCandidate.cleanup` が空)。`livecap-cli info` に `External model caches: N (size, outside the roots, never deleted)` 行を追加し、cli が使う repo (`KNOWN_MODEL_REPOS`) だけをサイズと `adopted` 付きで列挙する (`--as-json` は `external_model_caches: [{path, bytes, repo_id, adopted, missing}]`)。`adopted` は **その repo から作られる正本が全部 `models_root` にある** (判定は engine / translator と同じ `variant` / 必要ファイル込みの manifest 検証、`.nemo` は `model_store.validate_model_file()`) = livecap-cli としては外の copy が要らない、まで — 既定 HF cache は他アプリと共用で、同一 volume では hardlink なので削除の可否と解放量は利用者の判断 (ReazonSpeech は 1 repo から float32 / int8 の 2 正本を作るので、片方だけでは `adopted` にならない)。他アプリのモデルは出さない。既定 HF cache が root の中に解決する環境 (`HF_HOME` を `cache_root` に向けている) では root の中の旧配置として扱う
- **Migration**: none (自動)。外の cache を消したい場合は `livecap-cli info` の `adopted` (livecap-cli としては不要) を確認し、**他アプリが使っていないかを確かめて**手で削除する (cli は消さない)
...
- **After**: Voxtral は `fetch_repo_dir(allow_patterns=config / generation_config / preprocessor_config / tekken.json / model.safetensors.index.json / model-*.safetensors)` で必要ファイルだけを `<models_root>/mistralai--Voxtral-Mini-3B-2507/` へ置き、`from_pretrained(<dir>)` で読む (`save_pretrained` は呼ばない)。ReazonSpeech は int8 / float32 とも `reazon-research/reazonspeech-k2-v2` から `required_files()` の 4 ファイルだけを取る (float32 615 MB / int8 160 MB)。int8 の正本 dir は tarball 由来の `sherpa-onnx-zipformer-ja-reazonspeech-2024-08-01` から `<org>--<name>` 規則の `reazon-research--reazonspeech-k2-v2-int8` に変え、旧名の dir (root 直下 / `reazonspeech/` subdir) は初回ロードで取り込んで消す。 旧 int8 経路が `<cache_root>/downloads/` に残した tarball (713 MB) は、int8 の正本が validator を通った後 (cache hit または取得直後) に削除する (float32 だけを使う環境では `livecap-cli info` の残骸一覧に出るだけで触らない)。parakeet / reazonspeech / canary の override を削除し、正本は常に root 直下 (`<org>--<name>.nemo` はファイル)。engine subdir の重複と nested `.nemo` は初回ロードで validator を通る候補だけを正本の位置へ戻し、正本が valid になった後に重複を削除。validator を通らない root / dir は `<name>.invalid-<ts>` へ隔離 (削除しない)。取り込みと取得は destination 単位の同じ lock を共有する。既存の `save_pretrained()` 出力 / root 側 dir は adopt して再取得しない
...
#### `ModelManager.huggingface_cache()` を削除 ([#428])

- **Before**: `with manager.huggingface_cache() as hf_cache:` は実行時に `HF_HOME` を書き換えるコンテキストマネージャだった。**効いていなかった** — `huggingface_hub` は import 時に cache path を確定するので、後から `HF_HOME` を変えても既定の `~/.cache/huggingface` が使われ続ける。呼び出し元 5 engine のうち reazonspeech / voxtral は path を取り出すためだけに使い、canary / parakeet (NeMo は自前の cache 経路) では完全な no-op だった
- **After**: `ModelManager.get_huggingface_cache_dir() -> Path` (`<cache_root>/huggingface/hub`) に置き換え。**環境変数は触らず**、呼び出し側が `snapshot_download(repo_id, cache_dir=str(path))` のように**明示的に**渡す。宣言と実態が食い違う API を残さない
...
- **Before**: `configure_resources(cache_dir=...)` を指定しても、Qwen3-ASR (`qwen3asr` / `qwen3asr_large`) の 1.8 GB は **既定の `~/.cache/huggingface/hub` へ落ちていた**。`Qwen3ASRModel.from_pretrained(<repo ID>)` を `huggingface_cache()` (`HF_HOME` の実行時書き換え、効かない) で包んでいただけで、`configure_resources()` の readback が嘘になり、非 ASCII ユーザー名なら cache path が非 ASCII になり、容量管理の対象からも外れていた。加えて **marker を実ダウンロードの前に書いていた**ので、ダウンロードに失敗しても次回から「cached」と判定されていた
- **After**: `snapshot_download(repo_id, cache_dir=<管理 cache>, max_workers=1)` で **先に snapshot を管理 cache (`<cache_root>/huggingface/hub`) へ解決し、返ったローカル snapshot path を `from_pretrained()` へ渡す**。qwen-asr の `from_pretrained(**kwargs)` は `AutoModel` にしか渡らず `AutoProcessor` は `cache_dir` を受けないので、repo ID を渡す限り processor 側を管理下へ向けられない — ローカル path を渡せば model / processor が同じ snapshot を使う。**marker は解決に成功した後にのみ書き、中身は hub root からの相対 path と snapshot 内ファイルの一覧 (JSON)** にした。cache hit は「marker あり **かつ** **現在の**管理 cache 配下にその snapshot があり **かつ** 記録した全ファイルが実在する」ときだけ — `configure_resources(cache_dir=B)` へ変えた後に旧 root A の snapshot を使い続けない (相対 path は現在の root からしか解決しない) し、重みの削除・壊れた symlink も miss になる。それでも `from_pretrained()` が失敗したら marker を無効化して次回再解決する (self-heal)。`HF_HUB_OFFLINE=1` なら管理 cache だけから解決し、無ければ `LocalEntryNotFoundError` で **fail loud** — **既定 cache への silent fallback はしない** (実 Qwen で確認: 管理 cache へ複製 + 既定 cache を空 + offline でロード成功、管理 cache を空にすると失敗)。reazonspeech / voxtral は `get_huggingface_cache_dir()` へ置換 (階層が `<cache_root>/huggingface` から `<cache_root>/huggingface/hub` に変わる以外は挙動不変)。`livecap-cli info` / `--as-json` に **`HF cache`** (`huggingface_cache`) を追加し、production が `cache_dir=` に実際に渡す path を出す
- **Migration**: **既存の snapshot は自動移設しない。** 既定 cache に落ちている 1.8 GB はそのまま残り、次回ロード時に管理 cache へ**再ダウンロードされ得る** (`HF_HUB_OFFLINE=1` なら fail loud)。避けたい場合は **`~/.cache/huggingface/hub/models--Qwen--Qwen3-ASR-0.6B` ディレクトリ全体** (`blobs/` + `refs/` + `snapshots/`) を `<cache_root>/huggingface/hub/models--Qwen--Qwen3-ASR-0.6B` へコピーする。symlink が使える環境 (Linux / macOS / Developer Mode 有効の Windows) では `snapshots/` の各ファイルは `../../blobs/<hash>` への symlink なので、`blobs/` を省くとリンク切れになる (symlink を使えない Windows では実ファイルが `snapshots/` 直下にあるため `refs/` + `snapshots/` だけでも解決できるが、全体コピーが確実)。#428 以前の marker (`model=...` / `device=...`) は cache hit にならず再解決される。reazonspeech / voxtral の cache 階層が 1 段深くなるため、旧 `<cache_root>/huggingface/models--*` は使われなくなる (消してよい)
- **Note**: **`max_workers=1` は回避策である。** huggingface_hub 0.36.0 は fresh な cache dir へ複数 worker で落とすと、symlink 可否の判定 (`are_symlinks_supported()`) が判定前に `True` を仮置きするため thread 間で競合し、Windows (Developer Mode 無し) では `os.symlink` が `WinError 1314` で落ちる (実測)。1 worker なら degraded mode (実ファイルを snapshot 直下に置く) で正常に書ける。速度への影響は未計測で、安定性との trade-off として採用した。最新の 1.31.0 でも再現し、上流へ報告済み ([huggingface_hub#4915](https://github.com/huggingface/huggingface_hub/issues/4915))。上流で直れば `max_workers=1` は外せる。NeMo (canary / parakeet) の保存先は NeMo 自前の cache 経路のままで、本修正の対象外 ([#430] と同じ族)。非 ASCII 棚卸し表の `resources.model_manager.hf_home` 行は **`resources.model_manager.huggingface_cache_dir`** に置き換え、読み取りだけでなく **lock / blob / `.incomplete` / snapshot / refs の書き込み**をローカル mock Hub 相手に実測する probe (`huggingface_hub.snapshot_download.write`) を追加した。Qwen3-ASR の real_model probe は production と同じ手順 (管理 cache へ実体化 → `snapshot_download(local_files_only=True)` → `from_pretrained(<local>)`) に揃え、worker の `HF_HUB_CACHE` は**空の** scratch へ固定して silent fallback を検出できるようにした
- **Details**: [#428] / `docs/architecture/core-api-spec.md` / `docs/reference/cli.md`
...
- **After**: `translate.googleapis.com/translate_a/single?client=dict-chrome-ex&dt=t&dj=1` (JSON) へ切り替え (#443 では `client=at` を採ったが、翌日この IP から 429 になったため [#451] で `dict-chrome-ex` を既定にし、環境変数 `LIVECAP_GOOGLE_TRANSLATE_CLIENT` で上書きできるようにした)。**bot 判定は `TranslationError(reason="bot_challenge")` で即座に落ち、再送しない**。実測した bot 判定は 2 つの形があり、両方を捕まえる — (1) `www.google.com/sorry/` への redirect + reCAPTCHA、(2) **redirect 無しの 429 + 「automated queries」の Sorry ページ**。素の 429 (目印無し) は従来どおり retryable。**マーカーの比較は大文字小文字を無視する** (表記揺れで retryable に落ちて 3 回叩く再発を防ぐ)。**実在しない言語コード (`xx` / `jp`) と翻訳対象にならない特殊コード (`und` / `mul` / `zxx` / `mis` / private use) は送信前に `UnsupportedLanguagePairError`** — この endpoint は無効な `tl` でも 200 で原文を返すため、送ってからでは検出できない (`tlh` のように登録簿には実在するが Google が対応しない言語は送られ、endpoint 側で fail loud する)。**JSON の `sentences` に 1 つでも `{"trans": str}` でない要素があれば `layout_changed`** — 読み飛ばして部分翻訳を成功として返さない
...
- **Note**: **`client=gtx` は 2026-09-14 頃から遮断されている** (429 + "Sorry... automated queries"。複数 ISP から同じ curl で再現: eeeXun/gtt#43、noctalia-dev/official-plugins#64。手元でも `requests` から gtx は 429、`at` / `dict-chrome-ex` は 200)。**`at` / `dict-chrome-ex` は Google 自身のアプリ / Chrome 拡張の識別子**であり、gtx 遮断の意図を迂回する形になることは認識したうえで採用した。**`at` は 2026-09-15〜16 に同じ IP から 429 (同じ Sorry ページ) になった** ([#451]) — つまり**非公式経路の「復旧」は環境と時期に依存する**。次に既定が塞がれれば `bot_challenge` で落ちる (黙って原文にはならない)。`-m network` の smoke は通常 CI に含まれないので、リリース前に手元で 1 回通す ([#449])。**TLS 指紋の偽装は行わない** — bot 検知の回避で、識別子の選択とは性質が違う
- **Details**: [#442] / [#451] / `docs/troubleshooting/translation.md`
